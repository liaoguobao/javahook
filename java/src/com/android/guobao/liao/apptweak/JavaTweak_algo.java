...
public class JavaTweak_algo {
    ...
    static private void update1(Object thiz, ByteBuffer input) {
        update(thiz, JavaTweakUtil.bufferToByte(input));
    }
    ...
    static private byte[] digest(MessageDigest thiz) {
        ...
        JavaTweakBridge.writeToLogcat(Log.INFO, "MessageDigest::digest->{algo=%s, sign=%s}", thiz.getAlgorithm(), JavaTweakUtil.hexToString(hr, false));
        return hr;
    }
    ...
    static private void chooseProvider(Cipher thiz, Object initType, int opmode, Key key, AlgorithmParameterSpec paramSpec, AlgorithmParameters params, Object random) {
        ...
        JavaTweakBridge.writeToLogcat(Log.INFO, "Cipher::chooseProvider->{algo=%s, mode=%s, key=%s, iv=%s}", thiz.getAlgorithm(), mode, JavaTweakUtil.hexToVisible(key.getEncoded(), true), JavaTweakUtil.hexToVisible(iv, true));
    }
}
